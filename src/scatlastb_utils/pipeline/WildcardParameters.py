...
class WildcardParameters:
    ...
    This class is designed to parse wildcards and parameters from a configuration
    dictionary, map them to unique identifiers, and provide easy access to these
    wildcards across different datasets. It also supports writing the wildcard
    mapping to a specified output directory.
    """
    ...
    def __init__(
        ...
        """Initialize the WildcardParameters instance.
    ...
    def get_wildcards(
        ...
        df = self.subset_by_query(query_dict=query_dict, columns=[w for w in wildcard_names if w not in exclude])
    ...
    def get_paramspace(self, wildcard_names: list = None, exclude: list = None, **kwargs) -> Any:
    ...
    def get_from_parameters(
        ...
        try:
            ...
            if single_value:
                assert params_sub.shape[0] == 1, "More than 1 row after subsetting"
